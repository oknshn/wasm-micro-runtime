...
/* Thesis kernels (float32-based, timing only) */
void vec_abs(int *data, int len);
...
/* Fixed-point (q15) variants used on MCU/RISC-V targets.
 * These share the same pointer + length style API from the WASM side,
 * but reinterpret the buffers as int16_t/q15_t internally.
 */
 ...
 */
unsigned int thesis_get_ticks(void);

/* Legacy kernels (FIR filter, FFT) removed from this example. */
