...
/* Increase native runtime thread stack to handle large benchmark locals */
#define CONFIG_MAIN_THREAD_STACK_SIZE 16384
...
iwasm_main(void *arg1, void *arg2, void *arg3)
{
    ...
    /* Run native (non-WASM) thesis benchmarks first, using the same
     * kernels and inputs but without going through the WASM
     * interpreter. This provides a direct Zephyr baseline that can
     * be compared against the WAMR-based results.
     */
    thesis_native_bench_run();
